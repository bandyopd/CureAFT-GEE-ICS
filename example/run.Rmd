...
We demonstrate the proposed weighted AFT-GEE method in a scenario with a cure fraction.
We use a logistic model to generate the susceptibility status of each subject:
$$\mbox{logit}(p_{ik}) = \gamma_1X_{ik1} + \gamma_2X_{ik2} + \gamma_3X_{ik3} + \gamma_4X_{ik4},$$
where $\gamma_1 = 1, \gamma_2 = 0.6, \gamma_3 = 0.5$, and $\gamma_4 = -0.1$.
When a subject is assigned to the cured subgroup, the corresponding event time is set to infinity before applying censoring.
For subjects in the susceptible subgroup, the event time is generated from an accelerated failure time (AFT) model:
$$\log{T_{ik}} = \beta_0 + \beta_1 X_{ik1}+ \beta_2 X_{ik2} + \beta_3 X_{ik3} + \beta_4 X_{ik4} + \alpha_i + \epsilon_{ik},$$
...
$\alpha_i$ follows a normal distribution with mean 0 and variance 0.25, 
and $\epsilon_{ik}$ is the error term.
The true parameters are $\beta_0 = 2, \beta_1 = 0.3, \beta_2 = 0.4, \beta_3 = -0.2$, and $\beta_4 = -0.3$.
The intercept estimate is not reported in the simulation summaries.
...
$0.05\left[I_{K_i}(1 - \rho) + J_{K_i}\rho\right]$, where $I_{K_i}$ is a ${K_i}$-dimensional identity matrix 
and $J_{K_i}$ is a ${K_i}$ by ${K_i}$ matrix of 1s.
We considered three levels of $\rho$, namely 0, 0.3, and 0.6, 
representing no, moderate, and high within-cluster exchangeable residual dependence, respectively.
We generate independent censoring times at the subject level 
...
The arguments are in `dat_with_cure()` are as follow: 
  ...
  * `rho`: True residual within-cluster correlation parameter.
  * `c`: Rate parameter for the exponential censoring distribution.
  * `datadep`: Residual correlation structure used to generate the data.
  * `para1`: Regression coefficients in the latency component.
  * `para2`: Regression coefficients in the incidence component.
...
For example, the code below fits the mixture cure model with an exchangeable working correlation structure, utilizing the fixed-point accelerator `squarem`.
```{R, cache = TRUE}
...
The `cure_em()` function returns a list composed of seven elements, described as follows:
  ...
  * `surv_prob`: Estimated latency survival probabilities evaluated at the observed times.
  * `convergence`: Indicates whether the EM-type algorithm successfully converged.
  * `iter`: Represents the number of iterations required to achieve convergence.
  * `fpevals`: Number of evaluations of the fixed-point mapping.
...
```
In our experience, the EM-type estimating-equation algorithm may fail to converge for a small portion of the bootstrap samples.
To address this, one can remove clearly unconverged bootstrap estimates and
compute the empirical standard deviation of the remaining bootstrap estimates as the standard error.
...
kable(cbind(c("Incidence", "", "Latency", ""),
            c("PE", "SE", "PE", "SE"),
            round(tab, 3)),

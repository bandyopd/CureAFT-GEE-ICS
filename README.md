...
The example implementation uses existing packages including `MASS`,
`survival`, `aftgee`, `geepack`, `SQUAREM`, and `daarem`. The `MASS` and
`survival` packages are typically included with standard R
installations. The remaining packages can be installed using
...
`example` folder contains a demonstration of data generation, model
fitting, and bootstrap standard-error calculation, and the rendered
example can be accessed
[here](https://htmlpreview.github.io/?https://github.com/bandyopd/CureAFT-GEE-ICS/main/example/run.html)
for easy reference. The `code` folder includes the implementation of the
proposed EM-type algorithm and the fixed-point accelerators used in the
examples.
